...
from . import download, parse, examinee, filter as filt, enrich, export
...
def cmd_run_examinees(args: argparse.Namespace) -> int:
    DATA_RAW.mkdir(parents=True, exist_ok=True)
    OUTPUT.mkdir(parents=True, exist_ok=True)
    ...
    if not raw_path.exists():
        print(f"ERROR: --source file not found: {raw_path}", file=sys.stderr)
        return 2
    print(f"Using local examinee file: {raw_path}")

    df = examinee.parse_file(raw_path)
    print(f"Parsed {len(df):,} examinee records")
    ...
    counts = sd["affiliation_status"].value_counts().to_dict()
    print(
        f"Unaffiliated (salesperson examinees): {counts.get('UNAFFILIATED', 0)}  |  "
        f"Broker examinees: {counts.get('BROKER', 0)}"
    )

    sd = enrich.add_search_urls(sd)

    out_dir = OUTPUT / date.today().isoformat()
    paths = export.write_csvs(sd, out_dir, prefix="san_diego_examinees")
    for p in paths:
        print(f"Wrote {p}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    run = sub.add_parser("run", help="Download, filter, classify, export licensees")
    ...
    rune = sub.add_parser(
        "run-examinees",
        help="Parse a DRE Examinee List CSV, filter to San Diego, export",
    )
    rune.add_argument("--source", required=True, help="Path to examinee CSV")
    rune.set_defaults(func=cmd_run_examinees)
